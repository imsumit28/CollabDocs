...
function ErrorBanner({ message }: { message: string }) {
  return (
    <div className="mb-5 flex items-start gap-2.5 rounded-xl border border-red-100 bg-red-50 px-4 py-3" role="alert">
...
const inputClass =
  'h-[46px] w-full rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] px-4 text-[15px] text-[#0F172A] placeholder:text-[#94A3B8] outline-none transition-all duration-150 focus:border-[#3B82F6] focus:bg-white focus:ring-4 focus:ring-[#3B82F6]/[0.08]';
const primaryBtn =
  'flex h-[46px] w-full items-center justify-center gap-2 rounded-xl bg-[#0F172A] text-[15px] font-semibold text-white transition-all duration-150 hover:bg-[#1E293B] active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed';
    ...
    <div className="flex justify-between gap-2" role="group" aria-label="Verification code">
      {Array.from({ length: OTP_LENGTH }).map((_, i) => (
        <input
          ...
          onPaste={handlePaste}
          className={`h-[54px] w-full rounded-xl border bg-[#F8FAFC] text-center text-[22px] font-bold text-[#0F172A] outline-none transition-all duration-150 focus:bg-white focus:ring-4 focus:ring-[#3B82F6]/[0.08] disabled:opacity-50 ${
            invalid ? 'border-red-300 focus:border-red-400' : 'border-[#E2E8F0] focus:border-[#3B82F6]'
          }`}
        />
      ))}
    </div>
          ...
          <>
            ...
            <div className="mt-6 text-center text-[14px] text-[#64748B]">
              {cooldown > 0 ? (
                ...
                <button type="button" onClick={handleResend} disabled={loading} className="font-semibold text-[#3B82F6] hover:text-[#2563EB] transition-colors disabled:opacity-50">
                  Resend OTP
                </button>
              )}
            </div>
